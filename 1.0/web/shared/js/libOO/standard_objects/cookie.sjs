	...
	var $cookie={
		...
		*/
		setAuthUserId:function(user_id){
			...
			/* var domainParts = $server.requestServerUrl.match(/https?:\/\/([\w\-\.]+)/)
			var domain = domainParts[1].split(/\./);
			while (domain.length > 2) domain.shift(); */
			/* if (domain.length ==2){
				Myna.log("debug","domain",Myna.dump(domain) + Myna.dump(domainParts));
				var options = null//{domain:"." +domain.join(".")}
			} else */ 
			var options =null
