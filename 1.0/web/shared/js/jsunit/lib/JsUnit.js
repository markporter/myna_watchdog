...
function ComparisonFailure_toString()
{
    ...
    var expected= this.mExpected;
    var actual=this.mActual;
	...
	/*  else*/
   	if (Math.max( this.mExpected.length, this.mActual.length )>300){
...
function TestCase_runBare()
{
    ...
    {
        this.tearDown();
		  Myna.log("TEST","TEST ERROR: " + ex.message,Myna.formatError(__exception__));
        throw ex;
...
function TestCase_runTest()
{
    ...
    method = method.substring( method.lastIndexOf( "." ) + 1 );
	 $profiler.mark("running test case " + method);
    method = this[method];
    if( method )
        method.call( this );
    else
