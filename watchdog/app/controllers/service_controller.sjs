/* ========== Internal Functions ============================================ */
/* Function: _saveJson
	persists services and related tests and handlers to $FP.dir/services.json

	returns generated json
		
	*/	
	function _saveJson(params) {
		var services = $FP.getModel("Service");
		var ret = services.findBeans().map(function(s){
			var node =  s.getData(true).filter(function (v,k) {
				return ![
					"restarted",
					"test_status",
					"status"
			...
			node.Tests = s.Tests().map(function(t){
				var node =t.getData(true).filter(function (v,k) {
					return ![
						"status",
						"last_check",
						"fail_count",
		...
		new Myna.File($FP.dir,"services.json").writeString(json);
		return json;
	}
/* ========== Actions ======================================================= */
/* ---------- edit ------------------------------------------------------ */
...
/* ---------- exportJson ------------------------------------------------ */
	function exportJson(params){
		this.renderContent(this._saveJson(),"application/json",$server.hostName + "_watchdog_export.json")
	}
/* ---------- importJson ------------------------------------------------ */
